...
const UpdateOrder = ({ onclose, data, callFunc }) => {
    ...
    const productsId = data.cart_details.map(({productId, ...rest}) => productId)
    ...
    <div className="fixed top-0 right-0 left-0 bottom-0 w-full h-full z-[11] flex justify-center items-center bg-slate-200 bg-opacity-70">
      <div className="w-full bg-white shadow-md p-4 max-w-5xl rounded-lg">
        ...
        <div className='mb-4'>
            ...
            <select className='bg-green-500 p-1 rounded-sm text-white'
                onChange={handleSelected}
            >
                {
                    data?.status === 'WAITING' && (
                        <>
                            <option className='bg-white text-black' value='WAITING'>{statusOrder.map(status => status.value === "WAITING" && status.name)}</option>
                            <option className='bg-white text-black' value='CONFIRMED'>{statusOrder.map(status => status.value === "CONFIRMED" && status.name)}</option>
                            <option className='bg-white text-black' value='CANCEL'>{statusOrder.map(status => status.value === "CANCEL" && status.name)}</option>
                        </>
                    )
                }
            </select>
              ...
              {productInOrder.map((product, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  <td className="border border-gray-300 p-2 flex justify-center items-center">
                    <img src={product.image[0]} alt={product.name} width={45} height={45}/>
                  </td>
